...
router
  .group(() => {
    router.get('/list', [UsersController, 'index']).as('user.index')
    router.patch('/update/:id', [UsersController, 'update']).as('user.update')
    router.delete('/delete/:id', [UsersController, 'destroy']).as('user.destroy')
  })
  .prefix('users')
  .use(middleware.auth())
